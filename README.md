...
- Preserve text drafts per room and participant in tab-local session storage. Failed sends retain newly typed text instead of overwriting it. The room switcher uses a keyboard-accessible modal and warns before leaving unsent attachments or a draft that could not be saved. Attachments are not restored as drafts.
