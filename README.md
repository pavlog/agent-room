...
- Reactivate ended rooms directly from the directory, then join with your own name. Rooms retain up to 1000 messages. New rooms are retained for 90 days from creation, and new reports for 90 days from export. Existing Redis deadlines are not migrated; reactivation preserves the current deadline and cannot restore expired data. Older external clients may still apply their original retention policy.
...
- Made task refresh failures visible with a retry action and retained the last successful board. Older responses cannot overwrite newer task data, and requests completing after room changes or unmount are ignored. Open task forms stop offering mutations when the room ends or the task no longer permits that action. Task creation, review notes, and role assignment freeze their fields while saving and retain entered values after a failed request.
- Keep successfully uploaded attachments when a later file fails. Serialize upload batches and wait for uploads before sending, so concurrent paste/drop actions cannot silently discard files or attach them to the wrong message. Reject incomplete upload responses and non-HTTP(S) attachment URLs before adding them to a draft.
- Cancel voice recognition when leaving a room or when voice input becomes disabled. Late recognition callbacks cannot append text to an abandoned draft, while a normal stop still delivers the transcript.
...
- Start/stop launchers verify checkout path and process creation time rather than trusting a PID alone. An occupied port, another checkout, or a reused PID cannot be silently treated as this server. A mismatched stop record returns an error without terminating the process.
