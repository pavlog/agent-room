...
if (Test-Path '.local/server-process.json') {
    ...
    if (Test-LocalServerIdentity $info $record $PSScriptRoot) {
        ...
        if (-not $process) {
            Write-Output 'The recorded server process is no longer running.'
        } elseif ($process.StartTime.ToUniversalTime().Ticks.ToString() -ne $record.processStartTicks) {
            throw 'Process start time does not match the recorded launch; no process was stopped.'
        } else {
